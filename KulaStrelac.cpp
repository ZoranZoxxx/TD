...
KulaStrelac::KulaStrelac(int id, QGraphicsItem *roditelj)
    : Kula(id) /* MZ*/{
    this->id = id;
    ...
    for(int i = 0; i < BROJ_SMEROVA; i++) {
        for(int j = 0; j < BR_SIMULACIJA_GADJANJA; j++){
            ...
            if(j<10){
                QString putanja = QString(":/slike/slike/strelac/shooting %00000%1.png").arg(smer).arg(j);
                matricaSlika[i][j] = putanja;
            }
            else{
                QString putanja = QString(":/slike/slike/strelac/shooting %0000%1.png").arg(smer).arg(j);
                matricaSlika[i][j] = putanja;
            }
        }
    }
...
int KulaStrelac::identifikacijaKula(){
    return 1;
}

int KulaStrelac::getPrecnikDometa()
{
    return PRECNIK_KRUGA_DOMETA;

}

int KulaStrelac::getPolovinaSirine()
{
    return POLOVINA_SIRINE_KULE;
}

QGraphicsEllipseItem *KulaStrelac::getDomet()
{
    return domet;
}

void KulaStrelac::mousePressEvent(QGraphicsSceneMouseEvent *event){
...
void KulaStrelac::mouseReleaseEvent(QGraphicsSceneMouseEvent *event){
