...
Ork::Ork(QList<QPointF> nizTacaka, int brZivota, int id, QGraphicsItem *roditelj)
    ...
    for(int i = 0; i < BROJ_SMEROVA; i++) {
        for(int j = 0; j < BROJ_SIMULACIJA_KRETANJE; j++){
            ...
            case 7:
                smer = "se";
                break ;
            }                        //:/slike/slike/barbar/walking/walking e0002.png
            QString putanja = QString(":/slike/slike/harry the orc/walking/walking %20000%1.png").arg(j).arg(smer);
            //qDebug() << putanja;
    ...
    heltBar1 = new QGraphicsRectItem(this);
    heltBar1->setRect(x()+10,y(),30,5);
    heltBar1->setBrush(Qt::black);
    igra->scene()->addItem(heltBar1);
    heltBar2 = new QGraphicsRectItem(this);
    heltBar2->setRect(x()+10,y(),30,5);
    heltBar2->setBrush(Qt::green);
    igra->scene()->addItem(heltBar2);
...
void Ork::pomeriNapred(){
    if (igra->rezimPauze != 1) {
        ...
        if(this->getBrojZivota() <=0)
        {
            ...
            /*else if(igra->nivo > igra->brojNivoa)
            {
                igra->krajIgre();
            }*/
            return;
