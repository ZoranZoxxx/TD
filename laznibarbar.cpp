...
LazniBarbar::LazniBarbar(QList<QPointF> nizTacaka, int brZivota, int id, QGraphicsItem *roditelj)
    : LazniNeprijatelj(nizTacaka, brZivota){
    ...
    for(int i = 0; i < BROJ_SMEROVA; i++) {
        for(int j = 0; j < BROJ_SIMULACIJA_KRETANJE; j++){
            ...
            QString putanja = QString(":/slike/slike/barbar/walking/walking %20000%1.png").arg(j).arg(smer);
            //qDebug() << putanja;
